...
- **WHEN** 用户创建新主题并提供原始内容时
- **THEN** AI 服务必须接收原始内容进行翻译处理
- **AND** 必须返回翻译后的内容用于存储在 `translated_content` 字段
- **AND** 必须保持原始内容不变用于存储在 `content` 字段
- **AND** 必须处理翻译失败的情况，确保原始内容始终被保存
...
- **WHEN** 向测试端点发送需要翻译的内容时
- **THEN** 系统 SHALL 返回原始内容和翻译后的内容
- **AND** 验证翻译功能正常工作
...
- **AND** 提供重试机制或降级方案
- **AND** 确保主题创建流程不会因为翻译失败而中断
- **AND** 在翻译失败时将原始内容同时存储到 `content` 和 `translated_content` 字段
